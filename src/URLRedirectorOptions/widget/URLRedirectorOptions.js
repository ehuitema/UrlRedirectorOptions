...
define([
	...
	dom, dojoDom, domQuery, domProp, domGeom, domClass, domStyle, domConstruct, dojoArray, 
	lang, text, html, event) {
    'use strict';	
		
    // Declare widget"s prototype.
    return declare("URLRedirectorOptions.widget.URLRedirectorOptions", [_WidgetBase], {
        ...
        update: function (obj, callback) {
            ...
            if (this._contextObj !== null) {
				...
				//NOTES:
				//subscription on attr only is not triggered
				//use this.subscribe from _WidgetBase, it will remove old handles
				this._attrHandle = this.subscribe({
					guid: this._contextObj.getGuid(),
					/*attr: this.URLAttribute,*/
                        ...
                        if (objs.length === 1) {
							...
							this._attrHandle = this.subscribe({
								...
								callback: lang.hitch(this, function (guid, attr, attrValue) {
									...
									alert('attribute: ' + attr + ' changed to: ' + attrValue);
								});
							});							
...
*/
        ...
        _redirectTo : function(url, callback) {
			...
			if(url) {
				...
				if(this.CMDAttribute)
					this._command = this._contextObj.get(this.CMDAttribute);
				
				if(this._command == 'redirect')
				{
				...
				{
					this._windowRef = window.open(url, this.target, this.options);
				}
			}
        }
    });
});
require(["URLRedirectorOptions/widget/URLRedirectorOptions"], function () {
    "use strict";
});
